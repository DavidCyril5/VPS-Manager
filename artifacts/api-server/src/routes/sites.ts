...
// Detect whether this server uses Debian-style sites-available/sites-enabled or RHEL/Amazon-Linux conf.d
// Returns shell snippet that writes the nginx config to the correct location and reloads nginx
function nginxWriteScript(domain: string, configB64: string): string {
  return [
    `if [ -d /etc/nginx/sites-available ]; then`,
    `  echo '${configB64}' | base64 -d | sudo tee /etc/nginx/sites-available/${domain} > /dev/null`,
    `  sudo rm -f /etc/nginx/sites-enabled/${domain}`,
    `  sudo ln -sf /etc/nginx/sites-available/${domain} /etc/nginx/sites-enabled/${domain}`,
    `else`,
    `  echo '${configB64}' | base64 -d | sudo tee /etc/nginx/conf.d/${domain}.conf > /dev/null`,
    `fi`,
    `sudo nginx -t && sudo systemctl reload nginx`,
  ].join("\n");
}

// OS-aware nginx config removal — does NOT chain with rm -rf so it always completes independently
function nginxRemoveScript(domain: string): string {
  return [
    `if [ -d /etc/nginx/sites-available ]; then`,
    `  sudo rm -f /etc/nginx/sites-enabled/${domain}`,
...
// OS-aware nginx config read
function nginxReadScript(domain: string): string {
  return [
    ...
    `else`,
    `  echo "# Config not found for ${domain}"`,
    `fi`,
  ].join("\n");
}
...
function buildDeployParts(siteData: Record<string, unknown>): DeployParts {
  ...
  const setupNginx = nginxWriteScript(domain, nginxConfigB64);
...
router.delete("/sites/:id", async (req, res): Promise<void> => {
  ...
  if (server) {
    ...
    const pm2Cleanup = (siteType === "nodejs" || siteType === "python")
      ? `pm2 delete "${pm2Name}" 2>/dev/null || true; pm2 save 2>/dev/null || true`
      : "";
    // Use semicolons (not &&) so rm -rf always runs regardless of nginx reload outcome
    const parts = [
      pm2Cleanup,
      nginxRemoveScript(domain),
      `sudo rm -rf ${deployPath}`,
    ].filter(Boolean);
    const cleanupScript = parts.join("; ");
    await runSshCommand(sshOpts, cleanupScript, 60000).catch(() => {});
  }
...
router.put("/sites/:id/nginx-config", async (req, res): Promise<void> => {
  ...
  const script = nginxWriteScript(domain, configB64);
...
// --- Clear nginx logs on server ---
router.post("/sites/:id/clear-logs", async (req, res): Promise<void> => {
  const id = Number(req.params.id);
  ...
  // Truncate both nginx access and error logs; use || true so partial failure doesn't abort
  const cmd = [
